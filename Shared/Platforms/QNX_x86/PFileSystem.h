...
public:
                               ...
                               )
    {
		...
		dirent_extra *l_pDir_Extra = NULL;
		dirent_extra_stat *l_pDir_ExtraStat = NULL;
		...
		if (-1 == dircntl(l_pDir, D_SETFLAG, D_FLAG_STAT))
		{
			return;
		}
        ...
        {
			l_pDir_ExtraStat = NULL;
			for (l_pDir_Extra = _DEXTRA_FIRST(l_pDir_Entry); _DEXTRA_VALID(l_pDir_Extra, l_pDir_Entry); l_pDir_Extra = _DEXTRA_NEXT(l_pDir_Extra))
			{
				switch (l_pDir_Extra->d_type)
				{
					/* No data */
					case _DTYPE_NONE:
					default:
					...
					case _DTYPE_STAT:
						l_pDir_ExtraStat = (dirent_extra_stat*)l_pDir_Extra;
						break;
			...
			if (NULL == l_pDir_ExtraStat)
			{
				continue;
			}
			
            if (S_ISREG(l_pDir_ExtraStat->d_stat.st_mode))
            {
            ...
            else if (    (S_ISDIR(l_pDir_ExtraStat->d_stat.st_mode))
    ...
    static void Enumerate_Dirs(CBList<CWString*> *io_pList, const tXCHAR *i_pRoot)
    {
        ...
        while ((l_pDir_Entry = readdir(l_pDir)) != 0)
        {
            ...
            if (    S_ISDIR(l_pDir_ExtraStat->d_stat.st_mode)
                 && (0 != PStrCmp(l_pDir_Entry->d_name, "."))
                 && (0 != PStrCmp(l_pDir_Entry->d_name, ".."))
               )
            {
